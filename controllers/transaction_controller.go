...
func SeeOrder(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		if err := rows.Scan(&transaction.Id_Transaction, &transaction.Id_User, &transaction.Id_Promo, &transaction.Status, &transaction.Date); err != nil {
...
func SalesReport(w http.ResponseWriter, r *http.Request) {
	...
	start := r.URL.Query()["date1"]
	end := r.URL.Query()["date2"]

	rows, err := db.Query(`SELECT * FROM transactions WHERE date BETWEEN "` + start[0] + `" AND "` + end[0] + `"`)
	if err != nil {
		fmt.Println(err)
		PrintError(404, "Table Not Found", w)
		return
...
func TotalPrice(id_transaction int) int {
	...
	row := db.QueryRow(`SELECT SUM(c.price * a.quantity)-d.promo_amount
    ...
    JOIN promos d ON b.id_promo=d.id_promo
